...
export async function showEditPage(ctxInput) {
    ctx = ctxInput;

    const oldData = await getById(ctx.params.entryId);
    ctx.render(editTemplate(oldData));
}
...
async function onEditSubmit(ev) {
    ...
    await updateItem(entryId, formDataObj);

    ctx.page.redirect(`/details/${entryId}`);
}